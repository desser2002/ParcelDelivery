...
public class Shipment {
    ...
    public static Shipment restore(UUID id,
                                   String trackingNumber,
                                   UUID senderId,
                                   Address pickupAddress,
                                   String recipient,
                                   ...
                                   LocalDateTime createdAt,
                                   UUID vehicleId,
                                   UUID sortingCenterId) {
        return new Shipment(id, trackingNumber, senderId, pickupAddress, recipient,
            deliveryAddress, parcel, status, createdAt, vehicleId, sortingCenterId);
    }

    public static Shipment create(String trackingNumber,
                                  UUID senderId,
                                  Address pickupAddress,
                                  String recipient,
                                  Address deliveryAddress,
                                  Parcel parcel,
                                  LocalDateTime createdAt) {
        Shipment shipment = new Shipment(UUID.randomUUID(), trackingNumber, senderId, pickupAddress, recipient,
            deliveryAddress, parcel, ShipmentStatus.PENDING, createdAt, null, null);
    ...
    public void cancel() {
        if (!(this.status == ShipmentStatus.PENDING ||
            ...
            throw new IllegalStateException(
                "Cannot cancel shipment " + this.id + ": expected status PENDING " +
                    "or CONFIRMED or ARRIVED_AT_SORTING_CENTER  but was " + this.status
            );
        }
    ...
    public void markArrived(UUID sortingCenterId) {
        ...
        this.status = ShipmentStatus.ARRIVED_AT_SORTING_CENTER;
        this.sortingCenterId = sortingCenterId;
        events.add(new ShipmentArrivedAtSortingCenterEvent(id, sortingCenterId, Instant.now()));
    }
    ...
    public void markDelivered() {
        if (this.status != ShipmentStatus.ASSIGNED_FOR_DELIVERY) {
            throw new IllegalStateException(
                "Cannot mark Delivered  shipment: " + id
                    + " expected status ASSIGNED_FOR_DELIVERY but get: " + this.status
            );
        }

        this.status = ShipmentStatus.DELIVERED;
        events.add(new ShipmentDeliveredEvent(id, Instant.now()));
    }
